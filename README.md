...
- 使用 **Python + Gradio** 搭建中文客服聊天页面，页面采用简洁客服工作台风格，支持左侧历史会话和数据库片段回显。
...
- 支持 **按场景引导上传凭证**，用户提到破损、错发、少件等问题时再提示上传，输入区提供 `+` 号入口。
- 支持 **网页历史会话片段读取**，点击左侧历史记录后从数据库加载对应问答并展示到当前聊天页。
...
- 接入真实电商平台接口，例如订单中心、物流轨迹、退款系统和发票系统。
- 将当前轻量后台升级为更完整的客服工作台，支持工单分配、处理备注、状态流转和 SLA 超时提醒。
- 增加人工审核流程，把大模型生成的新问答审核后再写入正式知识库。
- 增加图片 OCR 或多模态识别，辅助判断破损、错发、少件等售后凭证。
- 增加权限控制、操作日志、异常监控和部署配置，满足正式上线要求。
